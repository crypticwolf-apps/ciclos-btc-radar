 ...
 */
/** Un cierre diario de la serie, con su día en formato YYYY-MM-DD. */
...
/**
 * Cierres diarios de BTC desde `startTime` (YYYY-MM-DD), en orden ascendente.
 * Es la materia prima tanto de los indicadores técnicos como del histórico de
 * halvings, que vive en `halvings.ts` con su propia cadena de respaldo.
 */
export async function getDailyCloses(startTime: string): Promise<DailyClose[]> {
  const rows = await fetchSeries(['PriceUSD'], startTime);
  const out: DailyClose[] = [];
  for (const row of rows) {
    const price = num(row, 'PriceUSD');
    if (price == null || price <= 0) continue;
    out.push({ t: Date.parse(row.time), day: row.time.slice(0, 10), price });
  }
  return out;
}

/** Un punto diario de una métrica cualquiera de la serie. */
...
/**
 * Varias métricas diarias en UNA sola petición, cada una con sus puntos válidos
 * en orden ascendente. La usan los bloques que necesitan una serie distinta del
 * precio (por ejemplo la divergencia on-chain, que la emplea como respaldo).
 */
export async function getDailyMetrics(
  metrics: string[],
  startTime: string,
): Promise<Record<string, DailyPoint[]>> {
  const rows = await fetchSeries(metrics, startTime);
  ...
  for (const key of metrics) {
    ...
    for (const row of rows) {
      ...
      points.push({ t: Date.parse(row.time), y: value });
    }
    if (points.length === 0) throw new Error(`Coin Metrics no sirvió la métrica ${key}`);
    out[key] = points;
  }
  return out;
}
...
export async function getTechnicals(): Promise<ProviderResult<Technicals>> {
  const r = await swr('cm:technicals', { ttlMs: 6 * 60 * 60_000, staleMs: 48 * 60 * 60_000 }, async () => {
    const series = await getDailyCloses('2010-01-01');
    ...
    const cycleStartIndex = series.findIndex((p) => p.t >= halvingMs);

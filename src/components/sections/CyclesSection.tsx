    ...
    <div className="space-y-3 sm:space-y-4">
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_300px]">
        <ChartCard
          ...
          subtitle="HistÃ³rico de suelos, expansiones y picos"
          info="La escala logarÃ­tmica permite comparar ciclos con precios muy diferentes."
          action={
          ...
          conclusion="Los halvings han marcado el ritmo histÃ³rico, aunque cada ciclo reduce su rendimiento porcentual y no garantiza que el patrÃ³n se repita."
        >
              ...
              <AreaChart data={data.cyclePrices} margin={{ top: 12, right: 4, left: 0, bottom: 2 }}>
                ...
                <Tooltip content={<ChartTooltip titleKey="year" renderBody={(point) => (
                  <div>
                    ...
                    <p className="text-xs text-muted">Ciclo {String(point.cycle)} Â· {String(point.phase)}</p>
                  </div>
                )} />} />
      ...
      <AccordionCard title="ComparaciÃ³n de rendimiento por ciclo" subtitle="De suelo a pico">
        <div className="grid grid-cols-2 gap-2 pt-3 sm:grid-cols-3 lg:grid-cols-5">
          {data.cycleComparison.map((cycle) => (
            <div key={cycle.cycle} className="rounded-xl border p-3 text-center" style={cycle.current ? { background: 'rgba(245,158,11,0.12)', borderColor: 'rgba(245,158,11,0.4)' } : { background: 'var(--surface)', borderColor: 'var(--surface-border)' }}>
              ...
              <p className="text-[10px] text-muted">{formatCompactFromUsd(cycle.min)} â†’ {formatCompactFromUsd(cycle.max)}</p>
            </div>
          ))}
        </div>
      </AccordionCard>

      <AccordionCard title="HistÃ³rico de halvings" subtitle="EmisiÃ³n, precio y retorno posterior">
        ...
        <table className="mt-3 hidden w-full table-fixed text-sm sm:table">
          ...
          <tbody>{data.halvings.map((halving) => {
            ...
            return <tr key={halving.year} className="border-b border-white/5">
              <td className="py-3 font-medium text-primary">{halving.year}</td><td className="py-3 text-center text-muted">{halving.reward}</td><td className="py-3 text-right text-secondary">{formatFromUsd(halving.priceAtHalving)}</td><td className="py-3 text-right font-medium text-bull">{halving.priceAfter18m ? formatFromUsd(halving.priceAfter18m) : 'En curso'}</td><td className="py-3 text-right font-bold text-bull">{ret == null ? 'â€”' : `+${ret.toLocaleString('es-ES')}%`}</td>
            </tr>;
          })}</tbody>
        </table>
      </AccordionCard>

      <AccordionCard title={`Fase actual: ${data.fase.nombre}`} subtitle="SeÃ±ales, riesgos y oportunidades">
  ...
  return (
    <div className="liquid-subcard rounded-xl p-3">
      ...
      <div className="mt-2 grid grid-cols-3 gap-2 text-center text-[10px] text-muted">
        ...
        <span>Retorno<strong className="mt-0.5 block text-xs text-bull">{ret == null ? 'â€”' : `+${ret.toLocaleString('es-ES')}%`}</strong></span>
      </div>
    </div>
  );
}

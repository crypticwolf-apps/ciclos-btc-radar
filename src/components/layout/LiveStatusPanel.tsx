...
export function LiveStatusPanel({
  ...
  return (
    <section
      ...
      aria-label="Estado de conexión y fuentes"
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="flex items-center gap-2 text-sm font-bold text-primary">
            ...
            {offline ? 'Conexión no disponible' : updating ? 'Actualizando datos' : 'Datos conectados'}
          </p>
          <p className="mt-1 text-xs text-muted">
            {lastUpdated ? `Última actualización ${timeAgo(lastUpdated)}` : 'Esperando la primera actualización'}
            {' · '}{source === 'live' ? 'fuentes reales' : source === 'stale' ? 'último dato válido' : 'modo de reserva'}
          </p>
        </div>
      ...
      {report ? (
          ...
          {report.providers.map((provider) => (
            <div key={provider.label} className="liquid-subcard min-w-0 rounded-xl px-2.5 py-2">
              ...
              <p className="mt-0.5 flex items-center gap-1.5 text-[10px] text-muted">
                ...
                {statusLabel(provider.status)} · {provider.latencyMs} ms
              </p>
            </div>
          ))}
        </div>
        ...
        <p className="mt-3 rounded-xl border border-white/10 px-3 py-2 text-xs text-muted">
          {health.isLoading ? 'Comprobando las fuentes…' : health.error?.message ?? 'No se pudo comprobar el estado.'}
        </p>
      )}
